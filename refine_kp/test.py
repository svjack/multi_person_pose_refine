...
from refine_kp.data_loader import data_loader_pre as data_loader, list_to_kp_map, kp_map_to_list, display, CenterGaussianHeatMap
...
def retrieve_conclusion(heatmap_producer):
    #[batch, 64, 64, 17]
    def max_xy(input):
...
def total_encoder_decoder_builder():
    ...
    return (input, gt_C, gt_heatmap, mask ,total_loss, opt, pred_hm, cfg)

def single_valid(cfg ,image, kp_map, resize_to = None, debug = False):
    ...
    if debug:
        ...
        pic_path = r"C:\Coding\Python\multi_person_pose_refine\{}_pics\{}.png".format(cfg.use_loss ,uuid1())
...
def train():
    input, gt_C, gt_heatmap, mask ,total_loss, opt, pred_hm, cfg = total_encoder_decoder_builder()
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess=sess, save_path="C:\\Coding\\Python\\RefineNet\\model_dir\\model_tiny_v2_1000")
        ...
        for i in range(int(1e6)):
            ...
            if i % 50 == 0 and i > 0:
                ...
                valid_total_loss, valid_hm = sess.run(
                    [total_loss, pred_hm], feed_dict={
                        input: batch_input,
                        gt_C: batch_kp_map_true,
                        gt_heatmap: batch_kp_heatmap_true,
                        mask: batch_kp_map_true_mask
                ...
                single_valid(cfg ,batch_input[0][..., :3], kp_map = kp_map,
                             resize_to = (256, 256), debug = True)
                print("continue !")

                if i % 1000 == 0:
                    saver.save(sess=sess, save_path=r"C:\Coding\Python\RefineNet\model_dir\model_tiny_v2_{}".format(i + 1000))
                    pass
