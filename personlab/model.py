...
def evaluate(model_func, data_generator, checkpoint_path, num_batches=-1):
    ...
    def filter_image_conclusion(input_result):
        indices = list(map(lambda tt2:tt2[0],filter(lambda t2: np.array(t2[1]).reshape([-1]).sum() > 0,enumerate(input_result["kp_map_pred"]))))
        req_result = dict()
        req_result["image"] = list(map(lambda indice: input_result["image"][indice], indices))
        req_result["kp_map_true"] = list(map(lambda indice: input_result["kp_map_true"][indice], indices))
        req_result["kp_map_pred"] = list(map(lambda indice: input_result["kp_map_pred"][indice], indices))
        return req_result
    ...
    with tf.Session() as sess:
        ...
        try:
            while num_batches != 0:
                ...
                times += 1
                if times % saved_times == 0:
                    ori_result_len = len(result["image"])

                    result = filter_image_conclusion(result)
                    new_result_len = len(result["image"])

                    print("ori vs filter {} {}".format(ori_result_len, new_result_len))

                    result_concat = {}
                    for key, val in result.items():
                        result_concat[key] = np.concatenate(val, axis=0)
                    with open(r"E:\Temp\conclusion_dir\{}.pkl".format(uuid1()), "wb") as f:
                        pickle.dump(result_concat, f)
                    print("have saved in times : {}".format(times))
